...
class getInput:
    ...
    def checkInput(self):
        ...
        parser.add_option("-v", "--verbose", action="store_true", dest="verbose", help="get status of every connection attempt [False]")
    ...
    def saveInput(self, parser, options):
        ...
        if options.output:
            if self.checkFile(options.output):
                ...
                print("[-] File format is not valid, outputting to default " + self.config["machines"])
        if options.threads:
    ...
    def addOption(self, option, parameter):
        if self.checkFile(option):
            ...
            self.config[parameter].append(option)

    # Read file and save its contents to the proper parameter
    def readFile(self, filename, parameter):
        realfile = open(filename, "r")
        [self.config[parameter].append(a) for a in realfile.read().splitlines()]
        self.config[parameter] = filter(None, self.config[parameter])
        realfile.close()

    # Check if File exists
    def checkFile(self, filename):
        try:
            re.search(r"(.*\.txt)", filename).group(1)
            return True
        except:
            return False
    ...
    def printInformation(self):
        ...
        print("\tMachines: " + str(len(self.config["machines"])))
        print("\tThreads: " + str(self.config["threads"]))
